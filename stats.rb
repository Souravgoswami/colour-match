...
def main
	...
	set title: 'Colour Match Statistics', width: $width, height: $height, fps_cap: $fps, background: 'white'
	...
	if score <  600 then iqtype, you_in = 'Very Low', 0
		...
		elsif score <  2000 then iqtype, you_in = 'Average', 2
		elsif score < 3000 then iqtype, you_in = 'Good', 3
		else iqtype, you_in = 'Excellent', 4
	end

	game_details = <<EOF
Colour Match is a game where you will have 2 cards.

The first card will always be black coloured, and it will
show you a colour name.

The second card will have random colours. You have
to guess if the first card's colour name matches the colour
of the second card.

How to play: Just tap yes or no accordingly to the match.
Useful Keys:
	Press left, a, 1, or j for yes.
	right, d, 3, ;, keys for no.
...
EOF
	...
	game_details.each_with_index do |l, i|
		game_details_texts << Text.new(l, font: 'fonts/Aller_Lt.ttf', x: 5, y: i * 17, size: 16, color: 'black')
	end
